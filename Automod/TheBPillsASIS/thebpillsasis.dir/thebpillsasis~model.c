...
char CommandName[] = "C:/AutoMod/14_5/bin/amod.exe";
...
MODELDLLEXPORT load*
newattrib(load* thisLoad, load* that)
{
	int	i0	= 0;

	if (!thisLoad->attribute) {
		thisLoad->attribute = (loadatt*)xmalloc(sizeof(loadatt));
	} else {
	}
	if (that) {
		*(thisLoad->attribute) = *(that->attribute);
	} else {
		memset(thisLoad->attribute, 0, sizeof(loadatt));
	}
	return thisLoad;
}

MODELDLLEXPORT void*
get_ldatt_address(load* thisLoad, int32 index, int32* d)
{
	void*	data	= INIT_Pointer;

	switch(index) {
	case 0:
		data = (void*)&thisLoad->attribute->am_model.am_V_id;
		break;
	case 1:
		data = (void*)&thisLoad->attribute->am_model.am_V_time;
		break;
	case 2:
		data = (void*)&thisLoad->attribute->am_model.am_V_type;
		break;
	case 3:
		data = (void*)&thisLoad->attribute->am_model.am_V_value;
		break;
	case 4:
		data = (void*)&thisLoad->attribute->am_model.am_V_dest;
		break;
	case 5:
		data = (void*)&thisLoad->attribute->am_model.am_V_dim;
		break;
	}
	return data;
}
...
MODELDLLEXPORT void
set_attdata(load* thisLoad)
{
	attribute*	att	= INIT_Pointer;

	am_model.am_V_id$att->data = (void*)&thisLoad->attribute->am_model.am_V_id;
	am_model.am_V_time$att->data = (void*)&thisLoad->attribute->am_model.am_V_time;
	am_model.am_V_type$att->data = (void*)&thisLoad->attribute->am_model.am_V_type;
	am_model.am_V_value$att->data = (void*)&thisLoad->attribute->am_model.am_V_value;
	am_model.am_V_dest$att->data = (void*)&thisLoad->attribute->am_model.am_V_dest;
	am_model.am_V_dim$att->data = (void*)&thisLoad->attribute->am_model.am_V_dim;
}
...
char*
AlignmentValue_valstrfunc(void* data)
{
	AlignmentValue* valptr = (AlignmentValue*)data;

	return alignment2str(*valptr);
}
...
{
	AMIntegerList** valptr = (AMIntegerList**)data;

	return AMIntegerListToStr(*valptr);
}
