...
// Generated:	Tue Jan 30 16:03:02 2024
...
Label4: ; // Step 4
		...
		{
			pushppa(thisObj, P_powder_arriving, Step 5, am_localargs);
			pushppa(thisObj, inqueue, Step 1, am2_Q_in);
			return Continue; // go move into territory
Label5: ; // Step 5
		}
		{
			if (waitfor(ToModelTime(10, UNITSECONDS), thisObj, P_powder_arriving, Step 6, am_localargs) == Delayed)
				return Delayed;
Label6: ; // Step 6
		}
		{
...
Label9: ; // Step 9
		...
		{
			return usefor(am2_R_mix, 1, thisObj, P_powder_arriving, Step 10, am_localargs, ToModelTime(normal1(am2_stream0, 30, 3), UNITSECONDS));
Label10: ; // Step 10
		}
		{
		...
		{
			clone(thisObj, 10, am2_P_mixing, NULL);
		}
	}
LabelRet: ;
	if (am_localargs)
